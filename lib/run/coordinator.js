...
export default class Coordinator {
  constructor({
    ...
    // initialize state and integrate reporter
    this.store = Store(create(State), x => {
      this.reporter.process(x);
      return x;
    });
    ...
    this.sockets
      .on('client/connect', meta => {
        this.store.connectBrowser(meta);
        this.sockets.send(meta.id, 'proxy:connected', this.proxy.url);
      });

    this.sockets // adapter API
      .on('adapter/connect', meta => this.sockets.send(meta.id, 'run'))
      .on('adapter/start', this.store.startTests) // remove this for speeeeed
      .on('adapter/test:end', this.store.updateTest)
      .on('adapter/end', meta => this.reporter.handleEnd(meta));
  }
